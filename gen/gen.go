...
func (g *Gen) Build(config *Config) error {
	...
	tmplFileName := filepath.Join(config.OutputDir, "tmpl.txt")
	...
	tmpl, err := os.Create(tmplFileName)
	if err != nil {
		return err
	}
	defer tmpl.Close()
	...
	// Write tmpl
	err = g.writeGoDocTmpl(packageName, tmpl, swagger, config)
	if err != nil {
		return err
	}

	log.Printf("create docs.go at %+v", docFileName)
	log.Printf("create template.json at %+v", tmplFileName)
...
func (g *Gen) writeGoDocTmpl(packageName string, output io.Writer, swagger *spec.Swagger, config *Config) error {
	generator, err := template.New("swagger_info").Funcs(template.FuncMap{
		"printDoc": func(v string) string {
			// Add schemes
			v = "{\n    \"schemes\": {{ marshal .Schemes }}," + v[1:]
...
var packageTemplateRaw = `{{ printDoc .Doc}}`
